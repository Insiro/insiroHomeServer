...
class UserControllerTest : AbsControllerTest("/users") {
    ...
    fun getUser() {
        ...
        Mockito.`when`(mockUserService.getUser(user.id!!)).thenReturn(Result.success(user))
    ...
    fun updateUser() {
        ...
        Mockito.`when`(mockUserService.updateUser(user.id!!, updateUserDTO))
            .then { user.name = newName; user }
